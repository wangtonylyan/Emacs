...
(defun my/prog/cpp/add-hook (func)
  (my/add-mode-hook "my/prog/cpp" func))

(defun my/prog/cpp/run-hook ()
  (my/run-mode-hook "my/prog/cpp"))

(dolist (mode '("c" "c++"))
  (my/add-mode-hook mode #'my/prog/cpp/run-hook))


(use-package cc-mode ;; c, c++, objc, java, idl, pike, awk
  :defer t
  :preface
  ...
  :init
  (my/prog/cpp/add-hook #'pkg/cc-mode/start)
  :config
...
(defun pkg/semantic/init ()
  ...
  ;; (my/prog/cpp/add-hook #'semantic-idle-scheduler-mode)
  ...
  (setq semanticdb-default-save-directory ;; 作为缺省路径，仅主动生成的数据库的文件才会保存于此
        ...
        semanticdb-project-root-functions (when (pkg/package/enabled-p 'projectile)
...
(use-package cedet
  ...
  (my/prog/cpp/add-hook #'pkg/cedet/start)
  :config
  (use-package ede
    :disabled
    :config
