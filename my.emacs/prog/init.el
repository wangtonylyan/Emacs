...
(use-package flycheck
  ...
  (use-package flycheck-irony
    ...
    :if (and (pkg/package/enabled-p 'irony)
             (pkg/package/enabled-p 'flycheck-irony))
    :init
  ...
  (use-package flycheck-ycmd
    ...
    :if (and (pkg/package/enabled-p 'ycmd)
             (pkg/package/enabled-p 'flycheck-ycmd))
    :init
  ...
  (use-package flycheck-pyflakes ;; add 'flycheck-pyflakes to checker list
    ...
    :if (and (pkg/package/enabled-p 'python)
             (pkg/package/enabled-p 'flycheck-pyflakes)
             (my/locate-exec "pyflakes")))
  (use-package flycheck-haskell
    :defer t
    :preface
    (defun pkg/flycheck-haskell/setup ()
      (flycheck-haskell-setup))
    :if (and (pkg/package/enabled-p 'haskell-mode)
             (pkg/package/enabled-p 'flycheck-haskell))
    :init
    (my/add-mode-hook "haskell" #'pkg/flycheck-haskell/setup))
  (use-package helm-flycheck
    :after (helm)
    :commands (helm-flycheck)
    :if (and (pkg/package/enabled-p 'helm)
             (pkg/package/enabled-p 'helm-flycheck))))
...
(use-package helm-gtags
  ...
  (defun pkg/helm-gtags/start ()
    (helm-gtags-mode 1))
  :if (and (pkg/package/enabled-p 'helm)
           (pkg/package/enabled-p 'helm-gtags)
           (my/locate-exec "gtags"))
  :init
