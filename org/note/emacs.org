...
+ edit ::
  - C-d :: (delete-char), without saving into kill ring
  - M-d :: (kill-word)
...
+ `t' :: true
*** others
+ `last-command' :: record the last command executed
+ `this-command' :: record the command now being executed, 当前正在执行的命令可指定其对外暴露的命令名称.
** function
*** type
...
+ (progn)
** macro
+ (when) :: if, without else part
  #+BEGIN_SRC emacs-lisp
    (when (<condition>)
      (<then>))
  #+END_SRC
+ (unless) :: if not, without else part
  #+BEGIN_SRC emacs-lisp
    (unless (<condition>)
      (<then>))
  #+END_SRC
